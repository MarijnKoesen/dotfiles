...
function! s:PHPCustomFolds() " {{{
    ...
    call s:PHPFoldPureBlock('^\s*\(class\|interface\)', s:FOLD_WITH_PHPDOC)
...
function! PHPFoldText() " {{{
    ...
    elseif strridx(getline(currentLine), "\/\*\*") != -1
        ...
        while currentLine < v:foldend
            ...
            if s:commentLine == 0 && !(strridx(line, "\*\/") != -1)
                let firstCommentline = line
                let s:commentLine = s:commentLine + 1
            else
                let s:commentLine = s:commentLine + 1
            endif
    ...
    if currentLine != v:foldstart
        let lineString = lineString . " " . firstCommentline
    endif   
